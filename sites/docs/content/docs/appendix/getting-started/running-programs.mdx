...
## When You Are Editing The CLI Itself

Run the checkout rather than whatever is on your `PATH`:

```bash
cargo run -p kira-cli --bin kira -- run examples/hello
```

Or install the checkout as the `dev` toolchain once with `knvm binstall`, after
which plain `kira` is the one you just built.
