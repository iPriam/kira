...
## CLI vs A Checkout You Are Changing

Use `kira` when you are exercising the installed or managed toolchain.

When you are changing the CLI itself, install the checkout as the `dev`
toolchain and keep typing `kira`:

```bash
knvm binstall
```

`binstall` builds the enclosing checkout and selects it, so every later `kira`
runs what you just compiled. To run one command without selecting anything:
