...
On the native-compiling backends (LLVM/native and hybrid), **direct FFI usage requires `@Native`**. The VM is different: it executes direct FFI through **LibFFI** (see [VM Dynamic FFI](#vm-dynamic-ffi-libffi) below), so the `@Native` requirement does not apply there.
...
Today, direct FFI usage includes direct calls or references to `@FFI.Extern` functions. On the native-compiling backends Kira reports `KSEM093` when a non-`@Native` declaration directly uses one. The diagnostic points at the FFI-bound symbol and asks you to mark the declaration `@Native` or move the direct extern call into a small native helper.

## VM Dynamic FFI (LibFFI)

The VM can invoke native functions **without a precompiled native backend and without `@Native`**, using a LibFFI-driven dynamic call path (the `kira_dynamic_ffi` package). The compiler carries FFI call signatures in bytecode metadata, the VM loads the target library, builds a LibFFI call interface from the signature, and dispatches the call at runtime.

```bash
kira run examples/ffi_dynamic_vm          # VM dynamic FFI via LibFFI
```

This lifts the `@Native` requirement on the VM specifically — direct extern calls run there as ordinary VM code. The native-compiling backends keep the `@Native` boundary because they marshal across a real compiled ABI edge.

libffi is pinned (libffi 3.5.2) and fetched like the managed LLVM toolchain:

```bash
knvm install libffi
```

Kira links libffi statically, so the archive is a build-time input for the
machine being built *for* — the command installs every published target, so a
cross build has the one it needs. See
[Installation](/docs/appendix/getting-started/installation).
