...
| hybrid | bytecode, native shared library, and a standalone executable | mixed `@Runtime` and `@Native` programs |
...
`kira build --backend hybrid` produces something the operating system starts, not just a bundle for `kira` to host. The deployment is four files in `.kira-build/`:
...
The executable is a copy of Kira's hybrid launcher: it loads the manifest beside itself and runs the program through both engines exactly as `kira run --backend hybrid` does in-process. Run it directly:
...
Everything after an explicit `.khm` path is passed to the program as its arguments; with no path argument, the launcher uses the one `.khm` beside the executable.

The directory is the deployment unit: move the four files together — under any names — and the program still runs. If several `.khm` files sit beside the executable, pass the one you mean; if none does, the launcher says so rather than guessing.
