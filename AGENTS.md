...
After pushing a branch with committed work, open a pull request yourself with `gh pr create` without waiting to be asked; do not leave pushed work without a PR. Open PRs in two stages and never skip the first: first open a PR within your own fork (`origin`) targeting the fork's `main`, then open a separate PR from the fork branch to `upstream` `main`. Never open a PR directly against `upstream` without the fork PR existing first. When tracking an open PR, periodically fetch status via background `gh` API calls. After pushing a branch or opening a draft PR, request reviews by commenting `@Codex review` and `@Coderabbit review`.
