...
For landing on the official Kira repo: ensure `origin` is a fork, never the official repo, and remember this across sessions; after opening the PR, trigger `@coderabbitai review` (and `@codex review` only when the change needs Codex's attention, since Codex uses limited quota); monitor via background shell until CI passes and reviews are satisfied. The upstream PR is merged by a maintainer of the official repo; do not merge it yourself unless you are operating as that maintainer (the repo owner), in which case you may merge the upstream PR once CI passes and reviews are satisfied.
