...
For landing on the official Kira repo: ensure `origin` is a fork, never the official repo, and remember this across sessions; after opening the PR, trigger `@codex review` and `@coderabbitai review`; monitor via background shell until CI passes and reviews are satisfied.
