...
//! types, free functions, and one shader made of compile-time options,
//! resource groups, and stages. There are no generics, no pointers, and no
//! recursion, and the only loop is `while` — so the tree stays flat enough
//! that every backend can walk it directly.
//!
...
pub use ast::{
    Access, BinaryOp, Block, Expr, Field, Function, Group, Import, Item, OptionDecl, Param,
    Resource, ResourceKind, Shader, StageDecl, StageWord, Stmt, TypeDecl, TypeRef, UnaryOp,
};
