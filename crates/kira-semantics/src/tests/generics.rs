...
#[test]
fn generic_struct_class_and_function_declarations_are_accepted() {
    assert!(
        codes("struct Box<Value> { let v: Value }\n@Main function main() { print(1) return }")
            .is_empty()
    );
    assert!(
        codes("class Box<Value> { let v: Value = 1 }\n@Main function main() { print(1) return }")
            .is_empty()
    );
    assert!(
        codes(
            "function id<Value>(v: Value) -> Value { return v }\n@Main function main() { print(1) return }"
        )
        .is_empty()
    );
}
...
#[test]
fn a_generic_function_infers_through_a_generic_constructor() {
    // The outer call learns `Value` from `Box<Value>`, while the constructor
    // learns its own `Value` from the nested `Score` value. This is the
    // inference path that explicit type arguments do not exercise.
    assert!(
        codes(
            "trait Scored { function score(borrow self) -> Int }\n\
             ...
             function unbox<Value: Scored>(value: Box<Value>) -> Value {\n\
                 return value.value\n\
             }\n\
             @Main function main() {\n\
                 let result = unbox(Box(value: Score(value: 8)))\n\
                 print(result.score())\n\
                 return\n\
             }"
        )
...
#[test]
fn a_generic_constructor_infers_labeled_fields_by_name() {
    assert!(
        codes(
            "struct Pair<Left, Right> {\n\
             ...
             @Main function main() {\n\
                 let pair = Pair(right: \"text\", left: 7)\n\
                 print(pair.left)\n\
                 print(pair.right)\n\
                 return\n\
             }"
...
#[test]
fn a_generic_class_inherits_and_infers_a_parameterized_parent() {
    assert!(
        codes(
            "class Grand<Value> {\n\
                 let value: Value\n\
...
#[test]
fn incomplete_generic_inference_is_reported_at_the_use_site() {
    assert!(
        codes(
        ...
        .iter()
        .any(|code| code == "KSEM316")
    );
    assert!(
        codes(
            "struct Pair<Left, Right> { let left: Left let right: Right }\n\
             @Main function main() {\n\
...
#[test]
fn conflicting_generic_inference_is_not_recovered_by_a_later_argument() {
    assert!(
        codes(
            "function same<Value>(left: Value, right: Value, again: Value) -> Value {\n\
                 return left\n\
             }\n\
             @Main function main() {\n\
                 let value = same(1, \"text\", 1)\n\
                 print(value)\n\
                 return\n\
             }"
        )
