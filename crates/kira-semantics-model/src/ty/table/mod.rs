...
pub struct TypeTable {
    ...
    arrays: ArrayTable,
    enums: EnumTable,
    distincts: DistinctTable,
    native_states: NativeStateTable,
    cells: CellTable,
    foreign_ptrs: ForeignPtrTable,
    // The packages the program was assembled from, by the name `import`
    // resolves against; the program's own files are no package.
...
impl TypeTable {
    ...
    /// The declared `distinct` types.
    pub fn distincts(&self) -> &DistinctTable {
        &self.distincts
    }

    /// The declared `distinct` types, for recording where a row was declared.
    pub fn distincts_mut(&mut self) -> &mut DistinctTable {
        &mut self.distincts
    }

    /// Declares a `distinct Name = representation` type, flattening a chain so
    /// the row records the scalar rather than the next link.
    ///
    /// Returns [`Type::Error`] when the id space is exhausted, for the reason
    /// [`TypeTable::array_of`] does: every caller is analysis, whose job is to
    /// keep going.
    pub fn declare_distinct(&mut self, name: String, representation: Type) -> Type {
        self.declare_distinct_owned(None, name, representation)
    }

    /// [`TypeTable::declare_distinct`], filed under the declaring package.
    pub fn declare_distinct_owned(
        &mut self,
        owner: Option<&str>,
        name: String,
        representation: Type,
        ...
        match self.distincts.declare_owned(owner, name, representation) {
            Some(id) => Type::Distinct(id),
            None => Type::Error,
        }
    }

    /// What `ty` *is* at run time: itself, unless it is a distinct type, in
    /// which case the representation it was declared over.
    ///
    /// The one question every layer below the type checker asks about a
    /// distinct type, and the only one it may ask. Total and non-recursive: a
    /// row's representation is already flattened by
    /// [`TypeTable::declare_distinct`].
    pub fn representation(&self, ty: Type) -> Type {
        match ty {
            Type::Distinct(id) => self.distincts.representation(id).unwrap_or(Type::Error),
            _ => ty,
        }
    }

    /// The representation of the distinct type `id`.
    pub fn distinct_representation(&self, id: DistinctId) -> Type {
        self.distincts.representation(id).unwrap_or(Type::Error)
    }

    /// Rewrites every type this table stores to its representation and drops
    /// the distinct rows.
    ///
    /// What makes a distinct type free: after this runs, the table holds no
    /// `Type::Distinct` anywhere, so a struct field, an array element, an enum
    /// payload, a cell, and a callback-state target each carry the scalar the
    /// author declared over — the same row a program that never wrote
    /// `distinct` would have. `kira-ir` calls it once, after rewriting the
    /// program's own types, which is why nothing below it needs an arm for a
    /// distinct type at all.
    pub fn erase_distinct_types(&mut self) {
        let representations: Vec<Type> = self
            .distincts
            .rows()
        ...
        self.structs.visit_field_types_mut(&erase);
        self.arrays.visit_elements_mut(&erase);
        self.enums.visit_payload_types_mut(&erase);
        self.cells.visit_inner_types_mut(&erase);
        self.native_states.visit_targets_mut(&erase);
        self.distincts = DistinctTable::new();
    }
    ...
    pub fn type_name(&self, ty: Type) -> String {
        match ty {
            ...
            // Named by itself, never by its representation. A diagnostic that
            // said `U32` where the author wrote `TabId` would name the one type
            // the refusal is *about* not being.
            Type::Distinct(id) => match self.distincts.get(id) {
                Some(def) => def.name.clone(),
                None => "<unknown distinct type>".to_owned(),
            },
    ...
    pub fn owns_heap(&self, ty: Type) -> bool {
        match ty {
            ...
            // Native state always is: the handle owns one reference to the
            // runtime's store, and the release that drops the last reference
            // destroys the boxed value.
            Type::String
            | Type::Array(_)
            | Type::Enum(_)
            | Type::Any
    ...
    /// Whether an ordinary read of a local of `ty` takes the value out of the
    /// local rather than copying it.
    ///
    /// A value with exactly one release has exactly one owner: a user `Drop`
    /// body runs once, and a native-state handle releases one reference once.
    /// Reading such a local moves the value to the reader, and the local's own
    /// release at scope end has nothing left to do.
    pub fn takes_on_read(&self, ty: Type) -> bool {
        matches!(ty, Type::NativeState(_)) || self.runs_user_drop(ty)
    }
