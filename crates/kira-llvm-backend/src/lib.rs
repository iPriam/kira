...
// The LLVM archives this crate is linked against, declared in the crate itself
// so the requirement travels with the rlib rather than only with the cargo
// directives that name it for a direct dependent. Generated by `build.rs` from
// the bundle's own `llvm-config`; see there for why both exist.
include!(concat!(env!("OUT_DIR"), "/llvm_link.rs"));
