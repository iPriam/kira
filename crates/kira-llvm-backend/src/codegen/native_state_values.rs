...
fn no_node_form(ty: Type) -> LlvmError {
    LlvmError::Unsupported(match ty {
        ...
        // Recovery is typed: `nativeRecover<T>` checks a stored identity
        // against `T`, and an erased value has none to check.
        Type::Any => "an `Any` inside native callback state",
        ...
        // A capture cell is shared mutable storage whose count one engine owns;
        // copying it would hand the other a second owner of that count.
        Type::Cell(_) => "a captured `var` inside native callback state",
...
impl Codegen<'_> {
    ...
    pub(super) fn encode_native_state_value(
        ...
        Ok(match ty {
            ...
            | Type::Task(_)
            | Type::Cell(_)
            | Type::NativeState(_) => return Err(no_node_form(ty)),
        })
    }
