//! Parity for array literals, indexing, `.count`, `.append`, and `for`-in.
//!
//! The wasm array is a header plus a *moving* item block, where the VM's is a
//! `Vec<Value>` on a slot table. Nothing below compares implementations — every
//! case runs the same source on the VM and on both address widths and asserts
//! they say the same thing, which is the only claim worth making.
...
/// The universal idiom: an empty literal grown by `append`. On wasm this is the
/// path that reallocates the item block, so it is also the growth test.
#[test]
...
/// A write through a path that mixes an index and two fields. On wasm the walk
/// is a GEP chain with a bounds check in the middle; if it rebuilt anything
/// instead of writing through, the read back would not see it.
#[test]
...
/// field, not share the handle. Both engines have to agree the original is
/// untouched.
#[test]
...
/// Out of range is a runtime trap on both engines, with the same message.
#[test]
...
/// A negative index is a *different* trap from one past the end. Both engines
/// have to draw that line in the same place.
#[test]
fn a_negative_index_traps_differently_from_out_of_bounds() {
    assert_parity(
        r#"
@Main
function main() {
