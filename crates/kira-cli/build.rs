fn main() {
    ...
    if target_os == "windows" {
        for symbol in process_symbols {
            println!("cargo:rustc-link-arg-bin=kira=/include:{symbol}");
            println!("cargo:rustc-link-arg-bin=kira=/export:{symbol}");
        }
    } else if target_os == "macos" {
        println!("cargo:rustc-link-arg-bin=kira=-Wl,-export_dynamic");
        for symbol in process_symbols {
            println!("cargo:rustc-link-arg-bin=kira=-Wl,-u,_{}", symbol);
        }
    } else if target_family == "unix" {
        println!("cargo:rustc-link-arg-bin=kira=-rdynamic");
        for symbol in process_symbols {
            println!("cargo:rustc-link-arg-bin=kira=-Wl,--undefined={symbol}");
        }
    }
}
